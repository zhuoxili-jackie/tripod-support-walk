...
> **状态（v1.14，2026-07-06，轻度压后腿内八；用户选"轻度"折中）**：用户反馈后腿摆动内八(hip 侧向内收、两膝靠近而轮距仍在)。量化：lowstep 后腿 hip |max| **16-17°**（前腿 8°、trotting 仅 7°）。**根因＝运动学非质心**（松 com-y 实测毫不影响内八）：后腿摆脚向 −Y（内侧）迈步 + 后腿较直的几何 → IK 逼出 hip 内收（knock-knee）；且**内八与前后摆耦合**（压 hip→侧向平衡转到机身→前后摆回来，同 v1.11 结论）。**无免费解**（松质心解不了，因非质心驱动）。**实测权衡（钉 hip pos 权重）**：50→内八16°/摆1.8cm、90→12°/2.8cm、150→10.5°/3.2cm、300→8.8°/4.0cm。**用户选"轻度"**：driver 加 `--hip-reg`(默认 **90**，stock 50)+ `--hipvel-reg`(**25** 阻尼 stiff-hip 抖动)，用库 `stateWeights`（base-ori 仍 500 保持平身、thigh/calf 仍 50）。0.05 右实测：后腿内八 **16-17→11.6°**、前腿 8→7°、前后摆 base_x 1.76→**2.76cm**（仍远优于原始 4.9）、pitch 1.35→3.5°、抬腿四脚均~2.1cm、镜像 ≤0.24mm、全收敛。8 版 CSV 已重生成。**待送审 commit**。
> **状态（v1.13，2026-07-06，修掉「摆左腿时机身前后摆动」）**：用户发现 v1.12 lowstep 版**右移时摆左前(FL)/左后(RL)腿机身前后(fore-aft)摆动**（左移镜像同理）。FK 量化坐实：per-swing base_x 摆幅 FL 2.0 / RL 1.9cm（全程 base_x 摆 4.9cm），三版（0703/0706/lowstep）都有，只因 lowstep 抬腿降到 2cm、竖直动作变小才**凸显**出来。**根因**：stock comTrack 把质心 x 钉死（权 1e6），摆腿时机身必须前后反冲以保持质心 x 不变。**修法（用户"不理会质心"口径下最对症）＝ 放松质心 x 跟踪**：给库 `createModel`/`createFootstepModels` 加**可选 `comWeights=None`**（默认 None＝各轴等权＝stock，**其它步态 byte 不变**，已验 walking `max|diff|=0`），driver 传 `comWeights=[--com-x-weight=0.1, 1, 1]`——**只松 x（前后），y（走行）/z（高度）仍钉死**。质心遂在前后向自由微动、机身不再反冲：**base_x 全程摆 4.9→1.8cm、FL 2.0→0.85 / RL 1.9→0.58cm**（且质心 x 漂移反而变小 1.6→0.14cm），走行/2cm抬腿/平身姿全不变。driver 相应从 `createWalkingProblem` 改为 driver 层 `createFootstepModels` 组装（为传 comWeights；结构与 stock 逐 byte 对齐：`ds+s0+s1+ds+s2+s3+[ds0]`），并改 per-leg 抬腿 `--front/hind-stepheight 0.016/0.026`（松 x 后前腿 tracking 偏高、后腿偏低，逐腿补偿使四脚均≈2.1-2.4cm）。8 版 CSV 已重生成、镜像 ≤0.35mm、FDDP 全收敛。**库改已 cp 到 build_conda**。**待用户 plot.py + 送审 commit**。
> **状态（v1.12，2026-07-06，用户否决 CoM-lean 的「抬屁股」，回到 0703 无质心 + 2cm 低抬腿）**：用户看 0706 交付（`quadruped_tripod_com_tilt_v*.csv`）后判定 **CoM 前倾送进后腿三角 = 抬屁股/姿态怪**，明确要求「**不必理会质心在不在身体内**，质心沿用 0703（stock、不考虑质心）」+「**腿抬前后均 2cm 即可**」，但**保留 0706 的迈步顺序**。落地 = **新驱动 `quadruped_tripod_lowstep.py`**（= `quadruped_tripod_reorder.py` 的 stock createWalkingProblem + 顺序 FR→RL→RR→FL，唯一改动 stepHeight 0.10→**0.02**）。**纯 driver、零库改、无 preshift/hold、无 stateWeights 改**。实测（0.05 右）：**抬腿 前 2.0 / 后 2.35cm**（footTrack 1e6 紧跟 2cm 目标；单一 stepHeight 故后腿略高 0.35cm）、**base_z 摆幅 2.1cm、pitch 摆幅 1.7°（三版中最平，抬屁股消失）** vs 0706 的 base_z 9.0cm / pitch 6.4°、0703 的 5.8cm / 1.8°；姿态保持 stock ~−1.09rad 自然前倾（0706 前倾到 −1.00）；左右镜像 ≤0.4mm；FDDP 全收敛 iters 15-26。**注意 `_verify_fall` 对本版误报 PASS**——它的 4cm 摆动阈值看不见 2cm 浅摆，把全程当四足支撑；真实后腿摆载荷与 0703 完全相同（stock CoM x≈0.06，后腿摆仍需对角前脚拉地），这是**用户已接受的 0703 剖面，不是新的稳定性收益**。8 版 CSV 已生成，**待用户 plot.py 肉眼 + 送审 commit**。
...
# ---- 生成：v1.13「0703 无质心 + 2cm 低抬腿 + 0706 顺序 + 前后防摆」交付（无抬屁股、机身不前后摆）----
python quadruped_tripod_lowstep.py                                # -> trajectory_tripod_lowstep_v0.05.csv
python quadruped_tripod_lowstep.py --speed 0.20 --direction left  # -> ..._lowstep_left_v0.20.csv
# 顺序 FR→RL→RR→FL（0706）；comRef=四脚平均（0703 无质心）；--com-x-weight 0.1 松质心 x 消机身前后摆（1.0=stock=会摆）；
# --hip-reg 90 轻压后腿内八 16→12°（升高=更少内八但前后摆回来，耦合）；--hipvel-reg 25 阻尼抖；
# per-leg 抬腿 --front/hind-stepheight 0.016/0.026 → 四脚均≈2cm；需库 comWeights（已 cp build_conda）
...
| **步态工厂（库；尽量别改）** | `bindings/python/crocoddyl/utils/quadruped.py`：`createWalkingProblem`(单腿摆动、rh→rf→lh→lf)、`createFootstepModels`(单相、driver 可直接调)、`createModel`(接受 `comTask`)。**可选参数（默认 None＝stock，其它步态 byte 不变）**：`stateWeights`(v1.9)、`comWeights`(v1.13，CoM 跟踪各轴权重，松 x 消机身前后摆)。改后必 cp 到 build_conda |
...
| **交付驱动（✅ v1.11：tilt≤5° + 抬腿 + 无内八；0706 CoM-lean 版，用户后否决抬屁股）** | `examples/quadruped_tripod_com.py [--speed --direction --hip-reg --margin ...]` → `trajectory_tripod_com_{,left_}tilt_v*.csv`（driver 组装、零库改；`--hip-reg 300` 默认钉死 hip 除内八→7°；用 v1.9 的可选 `stateWeights` 库参数）|
| **★交付驱动（✅ v1.14：0703 无质心 + 2cm 低抬腿 + 0706 顺序 + 前后防摆 + 轻压内八；当前推荐版）** | `examples/quadruped_tripod_lowstep.py [--speed --direction --com-x-weight --hip-reg --hipvel-reg --front/hind-stepheight]` → `trajectory_tripod_lowstep_{,left_}v*.csv`（driver createFootstepModels 组装；`--com-x-weight 0.1` 松质心 x 消前后摆；`--hip-reg 90` 轻压后腿内八 16→12°（代价前后摆 1.8→2.8cm，耦合、调 hip-reg 权衡）；`--hipvel-reg 25` 阻尼抖；抬腿四脚均≈2cm；用库 comWeights+stateWeights 参数；稳定性同 0703）|
...
- **v1.14（2026-07-06，轻度压后腿内八；用户选"轻度"折中；纯 driver、待送审 commit）**：
  - **现象（用户肉眼）**：lowstep 版**两条后腿摆动时内八**（hip 侧向内收、两膝靠近，而两轮间距仍在）。用户提示"之前 v1.11 是因为考虑质心才冲突，现在不考虑质心"——期待像修前后摆那样有免费解。
  - **★ 量化（scratchpad `hip_probe.py`：hip 关节 mean/|max|°，hip 列=CSV 第 8/11/14/17）**：lowstep 后腿 hip |max| **RL 16.1 / RR 17.0°**、前腿 FL 8.1 / FR 8.3°；对照 trotting（不内八）后腿仅 7.5/6.6、前腿 2.8/2.5°。
  - **★ 根因＝运动学，非质心（关键，回应用户）**：**松 com-y 跟踪（0.5→0.05）对后腿内八毫无影响**（16-17° 恒定）→ 内八不是质心跟踪逼的。真因：后腿摆脚**向 −Y（内侧）迈步** + 后腿几何较直 → footTrack(1e6) 逼 IK 用 hip 内收实现（redundancy 解成 knock-knee：hip 内收+calf 外展、脚位不变）。前腿踩高台腿伸展、同样 −Y 迈步只需 8°。**所以"不考虑质心"这次解不开**（与前后摆不同，前后摆是质心 x 钉死逼的、松 x 即解）。
  - **★ 内八与前后摆耦合（无免费解，同 v1.11 结论"侧向只能 hip 内收 or 机身侧倾二选一"）**：钉 hip 压内八 → 侧向平衡/迈步转移到机身 → **前后摆(base_x)回来**（−63°前倾几何把侧向 base 运动投影成前后）。实测松 com-x 更多**无法**re-absorb 这个耦合摆（hip 钉住后 comx 0.1/0.03/0.01 数值全同）。只钉后腿 hip → 内八**转移到前腿**（FR 8→15°）+ 摆更大。cost 手术换 com 活化/加 base pin → **段错误**（弃）。
  - **★ 权衡曲线（钉全 4 hip pos 权重，0.05 右，8-cycle）**：hip 50(stock)→内八16-17°/前后摆 whole 1.76cm；**80→12°/2.68**；100→11.5°/2.85；150→10.5°/3.23；200→9.9°/3.54；300→8.8°/4.0；500→7.3°/4.63（≈trotting 但摆基本全回来）。80 是曲线拐点（−4°内八只加 0.9cm 摆）。**AskUserQuestion 让用户选，用户选"轻度(hip~90)"**。
  - **落地（`quadruped_tripod_lowstep.py`，纯 driver、用现成库 `stateWeights` 参数、不再改库）**：加 `--hip-reg`(默认 **90**)、`--hipvel-reg`(**25**，阻尼 stiff-hip 抖动，只提 4 个 hip 速度、thigh/calf 速度保持 1 以免压没抬腿)、`--joint-reg`(50)。STATE_W=`[0]*3 base-pos + [500]*3 base-ori(仍平身不倾) + [hip,50,50]×4 + [10]*6 + [hipvel,1,1]×4`，传给 build_walking 的 createModel(ds)+createFootstepModels(fs)。
  - **★ 0.05 右实测**：后腿内八 **16-17→11.6°**、前腿 8→7°；前后摆 base_x whole **1.76→2.76cm**（RL 0.58→1.70、仍远优于原始 4.86）；pitch 1.35→**3.5°**、base_z 1.97→2.64cm（机身略活跃，是折中代价，仍无抬屁股）；抬腿 FL2.0/FR2.2/RL2.1/RR2.2cm；镜像 ≤0.24mm；FDDP 全收敛。**要更少内八可 `--hip-reg 150/300`（内八→10.5/8.8°，但前后摆→3.2/4.0cm）；要回最小摆用 `--hip-reg 50`。**
  - **本次改动（待送审 commit）**：`quadruped_tripod_lowstep.py`（hip-reg/hipvel-reg/joint-reg + STATE_W + 传 stateWeights）+ 8 版重生成 `trajectory_tripod_lowstep_*.csv` + 本文档。**未改库**（用 v1.9 已有的 stateWeights 参数）。新增工具 `hip_probe.py`（hip mean/|max|°）。
- **v1.13（2026-07-06，修掉 lowstep 版「摆左腿时机身前后摆动」；改库 comWeights + driver 组装；待送审 commit）**：
  - **现象（用户肉眼）**：v1.12 lowstep 版**右移时摆左前(FL)、左后(RL)腿，机身出现前后(fore-aft)摆动**；左移镜像同理（摆右侧腿）。
  - **★ FK 量化（scratchpad `probe_rock.py`：按摆动相切分，逐相 base_x/pitch 摆幅）**：per-swing base_x 摆幅 **FL 2.04 / RL 1.93cm**（FR 0.37 / RR 1.21，右侧腿小），全程 base_x 摆 **4.86cm**、一周期 8→38→8mm 一次前后 sway、**恰在左腿摆动收尾时急速回冲**。对照发现**三版（0703 baseline / 0706 com / lowstep）都有此 sway**（0703 FL 1.73、0706 甚至 RL 4.0cm+pitch6°）——不是 lowstep 引入的，只因 lowstep 抬腿从 10cm 降到 2cm、**竖直动作变小才把这横向前后摆凸显出来**。
  - **★ 根因**：stock `comTrack` 把质心 **x 钉死**（`ResidualModelCoMPosition` 权 1e6，各轴等权）。摆一条腿 = 该腿质量前后移动，为保持"质心 x 不变"，浮动基机身必须**前后反冲**（质心 x 钉住、机身随腿反向动）。**base 位置正则无效**（实测 stateWeights base-pos [W,0,W] 加到 2.5e5 仍几乎不动 sway，因它是被 com 钉逼出来的、非自由 slop）；改 supportKnots/stepKnots/stride 只**在腿间重新分配**不去根；driver 层 cost 手术（removeCost/addCost 换 com 活化或加 base pin）**段错误**（Python 绑定生命周期问题，弃）。
  - **★ 修法 = 放松质心 x 跟踪（用户"不理会质心"口径下最对症）**：机身反冲是为把质心 x 钉死；用户不在乎质心在哪、只要机身稳 → **松开 com 的 x 轴权重**，让质心前后自由微动、机身不必反冲。**改库**：`createModel`/`createFootstepModels` 加**可选 `comWeights=None`**（None＝各轴等权＝stock；给定则 comTrack 用 `ActivationModelWeightedQuad(comWeights**2)`）。**默认 None 使其它步态 byte 不变**（已验重生成 walking sc_v0.05 `max|diff|=0`）。**只改这两个方法、未碰 createWalkingProblem**（避免波及 trot/pace/bound 的共享代码）。cp 到 build_conda（§0.1 坑）。
  - **★ driver 改动（`quadruped_tripod_lowstep.py`）**：① 从 `createWalkingProblem` 改为 **driver 层 `createFootstepModels` 组装**（为能传 comWeights；结构逐 byte 复刻 stock：`ds + s0 + s1 + ds + s2 + s3 + [ds0]`，comRef/feetPos 就地累积，行数不变 1320/952/776/696）；② 新增 **`--com-x-weight`（默认 0.1）** = comWeights[0]（y,z 恒 1）；③ 抬腿改 **per-leg `--front/hind-stepheight 0.016/0.026`**（松 x 后前腿 tracking 偏高、后腿偏低，逐腿补偿使四脚峰值均≈2.1-2.4cm）。
  - **★ 0.05 右实测**：base_x 全程摆 **4.86→1.76cm**、**FL 2.04→0.85 / RL 1.93→0.58cm**（用户点名的两条左腿）；**质心 x 漂移反而降 1.61→0.14cm**（机身不再被每周期强行拉回，质心自然安顿）；抬腿 FL2.1/FR2.2/RL2.2/RR2.4cm、base_z 摆 1.97cm、pitch 摆 1.35°（比 v1.12 更平）；走行 y→−0.60 不变；镜像 ≤0.35mm；FDDP 全收敛 iters~14。**高速版 sway 随 stride 增大**（0.20：全程 3.83cm、FL/RL~1.7cm，仍远好于钉死基线）。
  - **本次改动（待送审 commit）**：库 `quadruped.py`（comWeights，源+cp build_conda）+ `quadruped_tripod_lowstep.py`（组装/com-x-weight/per-leg 抬腿）+ 8 版重生成 `trajectory_tripod_lowstep_*.csv` + 本文档。**新增复用工具（scratchpad，会过期）**：`probe_rock.py`（逐摆动相 base_x/pitch/base_y/roll 摆幅 + per-foot 汇总，量化机身摆动）、`probe_lift.py`（逐脚峰值抬升 + base_z/pitch 摆幅）。肉眼复核 `plot.py trajectory_tripod_lowstep_v0.05.csv --urdf pcb_v2/pcb_v2/urdf/pcb_v2.urdf --fps 100`。
- **v1.12（2026-07-06，回退 CoM-lean，改用「0703 无质心 + 2cm 低抬腿 + 0706 顺序」；待送审 commit）**：
  - **用户口径**：0706 的 CoM-lean 版（v1.9-1.11，`quadruped_tripod_com.py`）把 CoM 前倾送进后腿三角，导致**抬屁股（机身前倾 + base_z 大起伏）+ 姿态怪**。用户看片后决定**放弃送 CoM**：「不必理会质心在不在身体内，质心沿用 0703（那版没考虑质心）」，且「**腿抬高度前后均 2cm 即可**」，但**保留 0706 的迈步顺序 FR→RL→RR→FL**。
  - **★ 关键认识**：「0703 的质心approach + 0706 的顺序」= 现成的 `quadruped_tripod_reorder.py`（stock `createWalkingProblem`、无 CoM、reorder 绑定）**已经就是**，只差抬腿高度。故新驱动 = 它的近拷贝，唯一实质改动 stepHeight **0.10→0.02**。
  - **交付 `examples/quadruped_tripod_lowstep.py`**（stock createWalkingProblem，零库改，对称 cycle0 缓起步，stride×cadence 变速，右/左精确镜像，顺序 FR→RL→RR→FL）+ `--stepheight`（默认 0.02）。8 版 CSV `trajectory_tripod_lowstep_{,left_}v{0.05,0.10,0.15,0.20}.csv`（右/左 × 4 速度，1320/952/776/696 行）。
  - **★ 0.05 右实测（scratchpad `probe_lift.py`：逐脚相对自身站立面峰值抬升 + base_z/pitch 摆幅）**：抬腿 **FL 1.95 / FR 2.02 / RL 2.31 / RR 2.38 cm**（footTrack 1e6 紧跟 0.02 目标；**单一 stepHeight 无法前后独立**，后腿 tracking 略高 ~0.35cm，用户要更精确等值需切到 createFootstepModels 逐腿组装，代价是复杂度回升）。**base_z 摆幅 2.1cm、pitch 摆幅 1.7°** = 三版最平；对照 0703 baseline（front 11-12/hind 9cm、base_z 5.8cm、pitch 1.8°）、0706 tilt（front 19/hind 14cm、base_z **9.0cm**、pitch **6.4°**）→ **抬屁股彻底消失**，姿态保持 stock 自然前倾 pitch ~−1.09rad（0706 前倾到 −1.00）。**高速抬腿略升**（0.20 约 2.7-3.1cm，大 stride 微扰，仍远低于旧 14-19cm）。左右镜像 `_verify_mirror` ≤0.4mm/0.4mrad。FDDP 全收敛 iters 15-26。
  - **★ 稳定性 = 与 0703 完全同剖面（用户已接受）**：CoM 仍 stock 四脚平均 x≈0.06，后腿摆仍需对角前脚拉地，本质与 0703 一样"抬后腿静态会翻"（用户明确不再管）。**⚠ `_verify_fall.py` 对本版误报 PASS/FALL=0**——其 `SWING_THRESH=4cm` 看不见 2cm 浅摆，把全程当四足支撑 → 报 DS4 裕度 +0.19；**这是检测盲区，非真实稳定性提升**，度量本版稳定性须用别的手段（或临时调低 SWING_THRESH）。真实载荷同 0703（后腿摆 worst 拉力 ~30N）。
  - **本次改动（待送审 commit）**：新增 `examples/quadruped_tripod_lowstep.py` + 8 版 `trajectory_tripod_lowstep_*.csv` + 本文档。**不碰库、不碰 0703/0706 任何驱动或 CSV**（0706 的 `_com` 驱动与 tilt CSV 原样保留，可回退对比）。肉眼复核 `plot.py trajectory_tripod_lowstep_v0.05.csv --urdf pcb_v2/pcb_v2/urdf/pcb_v2.urdf --fps 100`。
