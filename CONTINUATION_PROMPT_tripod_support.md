...
> **状态（v1.10，2026-07-03 八次会话，见 §六）**：✅ 第 1 步（`_verify_fall.py`）· ✅ 第 2 步（改顺序 FR→RL→RR→FL）· ✅ 第 3 步（**用户改方向**：`quadruped_tripod_com.py` 限身体倾斜 **5°** + 高抬腿（前 15/后 10cm）+ 接受后腿摆摔由 RL 补偿；**改库** `createModel` 加可选 `stateWeights`，其它步态不变；已 commit `088f8ed`）。**⚠ 遗留（用户将新开对话修）**：tripod/walking **单腿摆步态有膝内八**（hip 内收 8~19°），**对角步态 trotting 不内八**（hip ~0）——根因 + 修法方向见 §六 v1.10。
...
| **主动送质心进三角 driver（🔄 v1.7 WIP 第三步根治）** | `examples/quadruped_tripod_com.py [--margin --stride --constraint]`（driver 组装、零库改、comTask→三角内可达点 + 预移/沉降相）|
