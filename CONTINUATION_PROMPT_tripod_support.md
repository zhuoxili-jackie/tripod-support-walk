...
| **★交付驱动（✅ v1.13：0703 无质心 + 2cm 低抬腿 + 0706 顺序 + 前后防摆；当前推荐版）** | `examples/quadruped_tripod_lowstep.py [--speed --direction --com-x-weight --front/hind-stepheight]` → `trajectory_tripod_lowstep_{,left_}v*.csv`（driver 层 createFootstepModels 组装；`--com-x-weight 0.1` 松质心 x 消机身前后摆 base_x 4.9→1.8cm；抬腿四脚均≈2cm；body 平 base_z 1.97cm/pitch 1.35°；用库 comWeights 参数；稳定性同 0703）|
