...
> **状态（v1.9，2026-07-03 八次会话，见 §六）**：✅ 第 1 步（`_verify_fall.py`）· ✅ 第 2 步（改顺序 FR→RL→RR→FL，缓解不根治）· ✅ 第 3 步（**用户改方向**：`quadruped_tripod_com.py` 限身体倾斜 **5°** + 腿自然（不内八）+ 高抬腿（前 15/后 10cm）+ 接受后腿摆摔由 RL 补偿；**改库** `createModel` 加可选 `stateWeights`，其它步态不变）——**改库+driver+CSV 待送审 commit**。v1.8「大倾斜 FALL=0 根治版」为不采用的对照存档。
